...
export function calendarRange(search,{owner=false}={}){
 ...
 const extension=search.get('extensionDays')||'0';if(!['0','1'].includes(extension))throw Error('Ungültige Mietdauer.');
 const dates=[];for(let date=start;date<end;date=plusDays(date,1))dates.push(date);
 return {month:start.slice(0,7),start,end,minDate,maxDate,dates,extensionDays:Number(extension)};
}
export function calendarData(store,range,{owner=false}={}){
 ...
 const days=range.dates.map(date=>{
  ...
  const free=store.available(date,plusDays(date,2+range.extensionDays));
 ...
 const result={month:range.month,reviewed,minDate:range.minDate,maxDate:range.maxDate,extensionDays:range.extensionDays,days};
