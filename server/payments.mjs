...
export function createPayments({store,stripe,origin,confirmationText}) {
 ...
 function applyRefund(b,r){
  ...
  if(r.status==='succeeded')enqueue(b.email,'Erstattung zu '+b.reference,`Die vollständige Erstattung zu ${b.reference} wurde von unserem Zahlungsanbieter ausgeführt. Die Buchung ist storniert. Die Gutschrift erfolgt über die ursprünglich verwendete Zahlungsart.\nFotobox Rendsburg`,null,'refund-'+b.id);
  if(r.status==='failed'||r.status==='canceled')enqueue(get('owner_email','uhighcauseidope@gmail.com'),'Fotobox Rendsburg: Erstattung fehlgeschlagen',`Die Buchung ${b.reference} ist storniert, aber die Erstattung ist fehlgeschlagen. Bitte den Vorgang in Stripe prüfen und die Rückzahlung mit dem Kunden klären.`,null,'refund-failed-'+r.id);
 });}
 ...
 async function createCheckout(b){return locked('checkout:'+b.id,async()=>{
  ...
  if(Date.now()-b.checkout_expires*1000>22*3600000){store.transaction(()=>{db.prepare('UPDATE bookings SET review_reason=? WHERE id=?').run('Zahlungsversuch ohne bestätigte Anbieterantwort. Bitte erneut abgleichen; bis zur Klärung bleibt die Box reserviert.',b.id);enqueue(get('owner_email','uhighcauseidope@gmail.com'),'Fotobox Rendsburg: Zahlungsversuch prüfen',`Bei ${b.reference} ist ein Zahlungsversuch ungeklärt. Bitte in der Verwaltung erneut abgleichen.`,null,'orphan-'+b.id+'-'+b.checkout_expires);});throw Error('Diese Zahlung muss persönlich geprüft werden. Bitte ruft uns an.');}
  const key='checkout-params-'+b.id+'-'+b.checkout_expires;
  let params=get(key);
  if(!params){const access=get('booking_access_'+b.id);params={mode:'payment',payment_method_types:['card',...(get('klarna_approved')&&b.customer_type==='private'?['klarna']:[])],client_reference_id:b.reference,customer_email:b.email,billing_address_collection:'required',locale:'de',metadata:{booking_id:b.id,attempt:String(b.checkout_expires)},payment_intent_data:{metadata:{booking_id:b.id,attempt:String(b.checkout_expires)},description:`Fotobox Rendsburg ${b.reference} – ${b.event_date}`,receipt_email:b.email},line_items:[{quantity:1,price_data:{currency:'eur',unit_amount:b.base_cents,product_data:{name:'Fotobox Rendsburg – eine digitale Fotobox',description:`Veranstaltung ${b.event_date}; bis zu 24 Stunden. Leistungen gemäß § 19 UStG umsatzsteuerfrei.`}}},...(b.extension_cents?[{quantity:1,price_data:{currency:'eur',unit_amount:b.extension_cents,product_data:{name:'Vereinbarte Verlängerung',description:'Bis zu weitere 24 Stunden.'}}}]:[]),...(b.travel_cents?[{quantity:1,price_data:{currency:'eur',unit_amount:b.travel_cents,product_data:{name:'Vereinbarte Lieferung und Abholung',description:`Vier Strecken; ${b.distance} km einfache Straßenentfernung${b.billed_distance==null?'':`, auf ${b.billed_distance} km aufgerundet`}; 10 km frei.`}}}]:[])],success_url:`${origin}/buchen/?angebot=${access}&zahlung=zurueck`,cancel_url:`${origin}/buchen/?angebot=${access}&zahlung=abgebrochen`,custom_text:{submit:{message:'Mit der Zahlung bucht ihr die bezeichnete Fotobox verbindlich. Es gelten die vorab angezeigten Buchungsbedingungen und die Widerrufsbelehrung.'}}};set(key,params);}
